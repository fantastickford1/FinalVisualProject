 ...
 * @author Karlos
 */
public class OpenFileBrowserFXMLController implements Initializable {
    ...
    private String allTextFromFile;
    ...
    private void openFile(){
        ...
        while(!selected.getValue().equals(parentDirectoryName)){
            ...
            fileAddress = selected.getValue() + "/" + fileAddress;
        }
        fileAddress = ".//" + fileAddress + selectedItem.getValue() + "//";
        File file = new File(fileAddress);
        fileNameField.setText(fileAddress);
        if(selectedItem.getValue().matches(".*\\b.txt\\b")){
            this.allTextFromFile = "";
            try {
                List<String> ls = Files.readAllLines(file.toPath());
                for (int i = 0; i < ls.size(); i++) {
                    this.allTextFromFile += ls.get(i) + "\n";
                }
            } catch (IOException ex) {
                Logger.getLogger(OpenFileBrowserFXMLController.class.getName()).log(Level.SEVERE, null, ex);
            }
        }else if (selectedItem.getValue().matches(".*\\b.png\\b")) {
            try{
                this.imagen = new Image(file.toURI().toString());
            }catch(Exception e){
                System.err.println("Can't open the image");
            }
        }else{
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setTitle("Can't open the selected file");
            alert.setHeaderText("Looks like you are trying to open a invalid file");
            alert.setContentText("Try to select a valid file please");
            alert.showAndWait();
        }
        /////////////////////////////////////////////////////////
        this.openFile.close();
    ...
    public String getReadedString(){
        return this.allTextFromFile;
    }
    
    public Image getReadedImage(){
        return this.imagen;
    }
